...
def detectTarget(cameraMatrix, dist, rMatrix, tvec, targetMarker, corners, markerIDs, zWorld=0.0):
    ...
    for i in range(len(markerIDs)):  # 遍歷探測到的marker ID,
        ...
        # marker的坐標從相機轉換到世界坐標np.linalg.inv求逆
...
if __name__ == '__main__':
    frame = cv2.imread(r'F:\PyCharm\Camera_calibration_GIT\001.png')
    try:
        npzfile = np.load('F:\PyCharm\Camera_calibration_GIT\class1\class_mtx.npz')
        mtx = npzfile['mtx']
        dist = npzfile['dist']
    except IOError:
